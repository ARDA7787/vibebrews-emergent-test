...
const faqs = [
  {
    ...
    answer: 'Absolutely not! VibeBrews uses conversational AI that understands natural language. Just describe your game idea like you\'re talking to a friend, and our AI will generate a fully playable HTML5 game for you.',
  },
  {
  ...
  {
    question: 'Is VibeBrews free to use?',
    answer: 'Yes, VibeBrews is free to download and use. You can create, play, and discover games without any cost. We\'re focused on democratizing game creation for everyone.',
  },
  {
    question: 'What platforms are supported?',
    answer: 'Currently, VibeBrews is available on Android via Google Play Store. All games created are HTML5-based, making them cross-platform compatible.',
  },
  {
  ...
  },
  {
    ...
    answer: 'Yes! VibeBrews supports up to 3 concurrent projects. You can work on multiple game ideas simultaneously, with background generation while you browse the feed.',
  },
]
...
function FAQItem({ faq, index }: any) {
  ...
  return (
    <motion.div
      ...
      viewport={{ once: true }}
      className="glass rounded-2xl overflow-hidden border border-vibe-teal/20 hover:border-vibe-teal/50 transition-colors"
      data-testid={`faq-item-${index}`}
    >
      <button
        onClick={() => setIsOpen(!isOpen)}
        className="w-full px-6 py-5 flex items-center justify-between text-left hover:bg-vibe-teal/5 transition-colors"
      >
        <span className="font-semibold text-lg pr-4">{faq.question}</span>
        <ChevronDown
          className={`w-5 h-5 text-vibe-teal flex-shrink-0 transition-transform duration-300 ${isOpen ? 'rotate-180' : ''}`}
        />
      </button>
      <motion.div
        initial={false}
      ...
      <div className="container mx-auto px-4 max-w-4xl">
        ...
        >
          ...
          <p className="text-xl text-gray-600">
            Everything you need to know about VibeBrews
          </p>
        </motion.div>
